...
function formatAccountBalance(balance: number, currencyCode: number): string {
  const currency = CCY[currencyCode];
  if (!currency) return `${balance.toFixed(2)} ${currencyCode}`;
...
export function ProfilePage() {
  ...
  async function handleDisconnect() {
    try {
      ...
      await refetchMono();
      setImportMsg("Monobank від'єднано");
      qc.invalidateQueries({ queryKey: ["mono-accounts"] });
      qc.invalidateQueries({ queryKey: ["mono-currency"] });
    } catch (err: unknown) {
      const msg =
        (err as { response?: { data?: { error?: { message?: string } } } })
          ?.response?.data?.error?.message ?? "Помилка від'єднання Monobank";
      setImportMsg(msg);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-xl space-y-5">
                ...
                {accounts.map((a) => (
                  ...
                  >
                    ...
                    <span className="font-medium">
                      {formatAccountBalance(a.balance, a.currencyCode)}
                    </span>
                  </div>
                ))}
              </div>
      ...
      <ConfirmDialog
        ...
        title="Від'єднати Monobank?"
        description="Токен буде видалено з профілю. Імпортовані раніше транзакції залишаться в системі."
