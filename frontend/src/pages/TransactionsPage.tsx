...
// Колонки таблиці з типовою, мінімальною та максимальною шириною (px).
const COLS = [
  { key: "date", label: "Дата", w: 120, min: 90, max: 200, align: "left" },
  { key: "desc", label: "Опис", w: 240, min: 120, max: 520, align: "left" },
...
export function TransactionsPage() {
  ...
  // Перетягування правого краю заголовка змінює ширину колонки (clamp min/max).
  ...
  // CSV-експорт через axios (з Bearer-токеном) + завантаження blob.
  ...
  // Групування транзакцій за місяцем (YYYY-MM) з підсумком по кожному.
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card px-4 py-3">
        ...
        <button
          onClick={() => setFilters({ from: "", to: "", type: "", category_id: "" })}
                    ...
                    {g.items.map((t) => (
                      <tr key={t.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="block max-w-48 truncate" title={t.description || undefined}>
                            {t.description || "—"}
                          </span>
                        </td>
