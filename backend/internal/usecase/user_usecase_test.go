...
func (m *mockUserRepository) Create(ctx context.Context, user *domain.User) error {
	...
	if args.Error(0) == nil {
		user.ID = 1 // імітуємо id, згенерований базою
	}
	return args.Error(0)
}
...
func TestUserUsecase_Register_EmailConflict(t *testing.T) {
	...
	// Реєстрація переводить конфлікт БД у зрозуміле «email вже зареєстрований».
...
func TestUserUsecase_Login_UserNotFound(t *testing.T) {
	...
	// Користувача немає, але назовні — узагальнена помилка.
	assert.ErrorIs(t, err, domain.ErrInvalidCredentials)
}
