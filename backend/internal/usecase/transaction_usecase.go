...
func NewTransactionUsecase(repo domain.TransactionRepository, catRepo ...domain.CategoryRepository) *TransactionUsecase {
	var categories domain.CategoryRepository
	if len(catRepo) > 0 {
		categories = catRepo[0]
	}
	return &TransactionUsecase{repo: repo, catRepo: categories}
}
...
func (uc *TransactionUsecase) Create(ctx context.Context, userID int64, input domain.CreateTransactionInput) (*domain.Transaction, error) {
	if err := uc.validateCategory(ctx, userID, input.CategoryID, input.Type); err != nil {
		return nil, err
	}
...
func (uc *TransactionUsecase) validateCategory(ctx context.Context, userID int64, categoryID *int64, txType domain.TransactionType) error {
	if categoryID == nil || uc.catRepo == nil {
		return nil
	}

	category, err := uc.catRepo.GetByID(ctx, *categoryID)
	if err != nil {
		return err
	}
