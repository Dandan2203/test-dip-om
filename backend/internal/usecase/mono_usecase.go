...
// Connect перевіряє токен через client-info, шифрує й зберігає його.
...
// Accounts повертає актуальні рахунки користувача з Monobank.
...
// token розшифровує збережений токен користувача.
...
// Import завантажує виписку рахунку Mono за період і додає нові транзакції.
// Суми зводяться до гривні; дублікати пропускаються. Повертає кількість доданих.
func (uc *MonoUsecase) Import(ctx context.Context, userID int64, accountID string, accountCurrency int, from, to time.Time) (int, error) {
	...
	_ = uc.repo.TouchImport(ctx, userID)
	return imported, nil
}

// expenseCategoryMap будує індекс назва→id для категорій витрат користувача.
...
// uahRate повертає коефіцієнт переведення валюти рахунку в гривню (1 для UAH).
