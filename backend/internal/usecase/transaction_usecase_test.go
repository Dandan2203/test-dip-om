...
func TestTransactionUsecase_Create_RejectsForeignCategory(t *testing.T) {
	categoryID := int64(9)
	ownerID := int64(99)
	repo := new(mockTransactionRepository)
	catRepo := new(mockCategoryRepository)
	catRepo.On("GetByID", mock.Anything, categoryID).Return(&domain.Category{
		ID: categoryID, UserID: &ownerID, Type: domain.CategoryExpense,
	}, nil)

	uc := NewTransactionUsecase(repo, catRepo)
	_, err := uc.Create(context.Background(), 5, domain.CreateTransactionInput{
...
func TestTransactionUsecase_Create_RejectsWrongCategoryType(t *testing.T) {
	categoryID := int64(9)
	repo := new(mockTransactionRepository)
	catRepo := new(mockCategoryRepository)
	catRepo.On("GetByID", mock.Anything, categoryID).Return(&domain.Category{
		ID: categoryID, Type: domain.CategoryIncome,
	}, nil)
...
func TestTransactionUsecase_Update_RejectsWrongCategoryType(t *testing.T) {
	categoryID := int64(9)
	existing := &domain.Transaction{
		ID: 10, UserID: 5, Type: domain.TransactionExpense, Amount: 100, TransactionDate: testDate,
	}
