...
const (
	maxChatMessageLen = 500 // макс. символів на один запит
	dailyChatLimit    = 50  // макс. на добу на користувача
)
...
func (h *ChatHandler) Chat(c *gin.Context) {
	...
	if result.Action != nil && result.Action.ActionType != "" {
		...
		if pending != nil {
			...
			resp["response"] = "Підготував дію. Підтвердіть її нижче."
			resp["intent"] = "ACTION"
			finalIntent = "ACTION"
		} else if clarify != "" {
	...
	if h.chatLogRepo != nil {
		logged, err := json.Marshal(gin.H{"response": resp["response"], "intent": finalIntent})
		if err != nil {
			slog.Warn("не вдалося підготувати лог чату", "error", err)
		} else if err := h.chatLogRepo.Record(ctx, &domain.ChatLog{
			UserID:   userID,
			Message:  req.Message,
			Intent:   finalIntent,
			Response: logged,
		}); err != nil {
			slog.Warn("не вдалося записати лог чату", "error", err)
		}
	}
...
func (h *ChatHandler) recordAction(ctx context.Context, log *domain.ActionLog) (*domain.ActionLog, error) {
	if err := h.actionRepo.Record(ctx, log); err != nil {
		return nil, fmt.Errorf("запис журналу дії: %w", err)
	}
	return log, nil
}
