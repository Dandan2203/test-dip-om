// Package mono — клієнт персонального API Monobank (api.monobank.ua).
package mono
...
// Account — рахунок або банка користувача.
type Account struct {
	ID           string   `json:"id"`
...
// StatementItem — запис виписки (одна транзакція).
type StatementItem struct {
	ID              string `json:"id"`
	Time            int64  `json:"time"`
	Description     string `json:"description"`
	MCC             int    `json:"mcc"`
	Amount          int64  `json:"amount"`          // у копійках, від'ємне = витрата
	OperationAmount int64  `json:"operationAmount"` // у валюті операції
	CurrencyCode    int    `json:"currencyCode"`
	Comment         string `json:"comment"`
}

// ClientInfo повертає дані клієнта та перелік рахунків.
...
// Statement повертає виписку за рахунком у заданому періоді (≤ 31 дня).
...
// Currency повертає курси валют з кешу (оновлення не частіше ніж раз на 30 хв).
func (c *Client) Currency(ctx context.Context) ([]CurrencyRate, error) {
	...
	if err := c.get(ctx, "/bank/currency", "", &rates); err != nil {
		// За наявності старого кешу повертаємо його, щоб не падати через ліміт.
		if c.rateCache != nil {
			return c.rateCache, nil
		}
		return nil, err
