// Package crypto — симетричне шифрування секретів (токени Mono, ключ ШІ) для зберігання в БД.
package crypto
...
// deriveKey зводить ключ довільної довжини до 32 байт через SHA-256.
func deriveKey(key string) []byte {
	sum := sha256.Sum256([]byte(key))
	return sum[:]
}

// Encrypt шифрує текст за алгоритмом AES-256-GCM і повертає base64(nonce|ciphertext).
...
// Decrypt розшифровує рядок, отриманий від Encrypt.
