...
func (c *Client) post(ctx context.Context, path string, body, result any) error {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		if err != nil {
			lastErr = fmt.Errorf("ai client: do request: %w", err) // мережна помилка
			continue
		}
		...
		if resp.StatusCode >= 500 {
			...
			resp.Body.Close()
			lastErr = fmt.Errorf("ai client: status %d: %s", resp.StatusCode, b) // 5xx
			continue
		}
		if resp.StatusCode >= 400 {
			b, _ := io.ReadAll(resp.Body)
			resp.Body.Close()
			return fmt.Errorf("ai client: status %d: %s", resp.StatusCode, b) // 4xx
		}
