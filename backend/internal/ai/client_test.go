...
func TestClient_RetriesOn5xxThenSucceeds(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if n < 3 {
			w.WriteHeader(http.StatusInternalServerError) // дві перші спроби — 5xx
			return
		}
		w.WriteHeader(http.StatusOK)
...
func TestClient_NoRetryOn4xx(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		atomic.AddInt32(&calls, 1)
		w.WriteHeader(http.StatusBadRequest) // 4xx — помилка клієнта, повтор безсенсовий
	}))
	defer srv.Close()
...
func TestClient_RespectsContextCancellation(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		atomic.AddInt32(&calls, 1)
		w.WriteHeader(http.StatusInternalServerError) // змушує піти на backoff між спробами
	}))
	defer srv.Close()

	// Контекст скасовується раніше, ніж устигнуть усі спроби з backoff.
