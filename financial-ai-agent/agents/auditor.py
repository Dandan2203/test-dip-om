...
def audit(req: AuditRequest) -> AuditResponse:
    ...
    categorized_amounts = [t.amount for t in expense_txs if t.category_name]
    ...
    for cat, txs in by_category.items():
        for t in txs:
            amounts = [other.amount for other in txs if other.id != t.id]
            if len(amounts) < 3 and not t.category_name:
                amounts = categorized_amounts
            if len(amounts) < 3:
                continue
